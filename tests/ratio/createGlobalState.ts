...
const createGlobalStateCall = async (accounts: Accounts, user: User) => {
  ...
  txn.add(
    programStablePool.instruction.createGlobalState(
      ...
      accounts.mintUsdr.bump, // prev: mintUsdNonce
      ...
      new BN(constants.GLOBAL_DEBT_CEILING),
      {
        accounts: {
          ...
          mintUsdr: accounts.mintUsdr.pubKey,
...
export const createGlobalStatePASS = async (
  ...
  console.log(
    "mint: ",
    accounts.mintUsdr.pubKey,
    "\n string:",
    accounts.mintUsdr.pubKey.toString()
  );
  ...
  assert(
    accounts.global.state.mintUsdr.toBase58() ==
      accounts.mintUsdr.pubKey.toBase58(),
    "\n USDr mint is not correct"
  );
  assert(
