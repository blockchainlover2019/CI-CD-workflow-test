...
describe("admin-panel", function () {
  ...
  it("Pause Ratio in Emergency state", async () => {
    // TXN 1
    ...
    assert(globalState.paused == 1, "Ratio Not Paused");
  });

  it("Resume Ratio in Emergency state", async () => {
    // TXN 1
    const txn1 = await stablePoolProgram.rpc.toggleEmerState(
      0, // paused
      {
    ...
    assert(globalState.paused == 0, "Ratio Not Resumed");
  });

  it("FAIL CASE: Pause Ratio in Emergency state with other user", async () => {
    // TXN 1
    const txn1 = await stablePoolProgram.rpc.toggleEmerState(
      1, // paused
      {
