...
pub mod stable_pool {
    ...
    pub fn create_global_state(
        ...
        ctx.accounts.global_state.mint_usdr = ctx.accounts.mint_usdr.key();
        ctx.accounts.global_state.mint_usdr_nonce = mint_usdr_nonce;
...
#[derive(Accounts)]
#[instruction(global_state_nonce: u8, mint_usdr_nonce: u8)]
pub struct CreateGlobalState<'info> {
    ...
    #[account(
        init,
        payer = authority,
        mint::decimals = USDR_DECIMALS,
        mint::authority = global_state,
        seeds = [MINT_USDR_SEED],
        bump,
    )]
    pub mint_usdr: Account<'info, Mint>,
