...
pub mod stable_pool_faucet {
    ...
    pub fn create_state(ctx: Context<CreateFaucetState>, state_nonce:u8,  mint_usdc_usdr_lp_nonce:u8, mint_eth_sol_lp_nonce:u8, mint_atlas_ray_lp_nonce:u8, mint_samo_ray_lp_nonce:u8) -> ProgramResult { process_create_state(ctx, state_nonce,  mint_usdc_usdr_lp_nonce, mint_eth_sol_lp_nonce, mint_atlas_ray_lp_nonce, mint_samo_ray_lp_nonce) }
    pub fn faucet_usdc_usdr_lp(ctx: Context<FaucetUsdcUsdrLp>, state_nonce: u8, mint_lp_nonce: u8) -> ProgramResult { process_faucet_usdc_usdr_lp(ctx, state_nonce, mint_lp_nonce) }
