...
import { borrowUSDr, createGlobalState, createTokenVault, createUserTrove, depositCollateral,  repayUSDr, TEST_LP_TOKEN_MINT, withdrawCollateral } from '../actions';
...
const PageHome : React.FC = () => {
  ...
  async function borrowUSDrUI() {
    if(wallet.connected){
      const demoLog = await borrowUSDr(connection, wallet, 50 * 1000000, TEST_LP_TOKEN_MINT);
      setDispInfo(demoLog);
    }
    else{     setDispInfo("connect your wallet");    }
  }
  async function repayUSDrUI() {
    if(wallet.connected){
      const demoLog = await repayUSDr(connection, wallet,  20 * 1000000, TEST_LP_TOKEN_MINT);
      setDispInfo(demoLog);
    }
    else{     setDispInfo("connect your wallet");    }
  }
